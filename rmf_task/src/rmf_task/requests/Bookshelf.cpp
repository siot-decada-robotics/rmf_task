...
//==============================================================================
class Bookshelf::Model : public Task::Model
{
public:
  ...
  Model(
    ...
    const rmf_traffic::Trajectory& bookshelf_path,
  ...
  _end_waypoint(end_waypoint)
{
  ...
  const auto& bookshelf_start_time = bookshelf_path.begin()->time();
  const auto& bookshelf_finish_time = *bookshelf_path.finish_time();

  _invariant_duration =
    bookshelf_finish_time - bookshelf_start_time;

  // Compute battery drain over invariant path
  const double dSOC_motion =
    _parameters.motion_sink()->compute_change_in_charge(bookshelf_path);
...
//==============================================================================
std::optional<rmf_task::Estimate> Bookshelf::Model::estimate_finish(
  const State& initial_state,
  const Constraints& task_planning_constraints,
  const TravelEstimator& travel_estimator) const
{
  ...
  if (_start_waypoint != _end_waypoint)
  {
    ...
    // end of bookshelf scanning trajectory to its end_waypoint. We currently define the
...
//==============================================================================
rmf_traffic::Duration Bookshelf::Model::invariant_duration() const
{
  return _invariant_duration;
}

//==============================================================================
class Bookshelf::Description::Implementation
{
public:
...
//==============================================================================
std::shared_ptr<Bookshelf::Description> Bookshelf::Description::make(
  std::size_t start_waypoint,
  std::size_t end_waypoint,
  const rmf_traffic::Trajectory& bookshelf_path)
{
  std::shared_ptr<Bookshelf::Description> bookshelf(new Bookshelf::Description());
  bookshelf->_pimpl->start_waypoint = start_waypoint;
  bookshelf->_pimpl->end_waypoint = end_waypoint;
  bookshelf->_pimpl->bookshelf_path = bookshelf_path;
...
//==============================================================================
Task::ConstModelPtr Bookshelf::Description::make_model(
  ...
  const Parameters& parameters) const
{
  ...
  return std::make_shared<Bookshelf::Model>(
    earliest_start_time,
    parameters,
    _pimpl->bookshelf_path,
    _pimpl->start_waypoint,
    _pimpl->end_waypoint);
}

//==============================================================================
auto Bookshelf::Description::generate_info(
  const State&,
  const Parameters& parameters) const -> Info
{
  ...
  return Info{
    "Bookshelf " + standard_waypoint_name(graph, _pimpl->start_waypoint),
    "",
  };
}

//==============================================================================
std::size_t Bookshelf::Description::start_waypoint() const
{
  return _pimpl->start_waypoint;
}

//==============================================================================
std::size_t Bookshelf::Description::end_waypoint() const
{
  return _pimpl->end_waypoint;
}

//==============================================================================
ConstRequestPtr Bookshelf::make(
  std::size_t start_waypoint,
  std::size_t end_waypoint,
  const rmf_traffic::Trajectory& bookshelf_path,
  const std::string& id,
  ...
  bool automatic)
{
  const auto description = Bookshelf::Description::make(
    start_waypoint,
    end_waypoint,
    bookshelf_path);
